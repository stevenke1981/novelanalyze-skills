...
{
  "source": "渡口",
  "author": "可選。若知道作者就填，會納入圖像提示詞禁用檢查",
...
| 頂層欄位 | 必填 | 說明 |
| --- | --- | --- |
| `source` | 是 | 書名/篇名，報告標題用；也會加入圖像提示詞禁用檢查 |
| `author` | 否 | 作者名。若提供，同樣不得出現在圖像提示詞 |
...
| `image.prompt` | string | **英文** | 卡通角色設定圖；**禁止出現人名、書名、作者名**。少於 2 個字的中文名稱不檢查 |
