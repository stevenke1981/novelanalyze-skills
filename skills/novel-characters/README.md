...
將小說或短篇故事轉換成可直接投入角色設計、配音、真人選角與影像製作流程的角色設定集。
...
- **可攜式成果**：`cast.json`、真人版 JSON、Markdown 與可離線開啟的 `report.html`。
- **圖片生成**：目前環境有圖像工具時，可產生卡通三視圖與真人身份固定圖片組；沒有工具時仍交付完整提示詞與狀態。
...
OpenCode 可完整執行角色分析、驗證與報告產生；若目前環境沒有圖像工具，技能會交付三視圖與真人圖片組提示詞，並明確標示 `NOT_RUN`。
...
6. 使用者要求真人版時，另建 `*-live-action.json`，不覆寫卡通欄位。
7. 真人圖片組先生成並核准身份固定板，再用同一參考圖生成肖像、臉部角度、三視圖、表情、服裝與電影關鍵畫面。
...
真人版 sidecar 預設只處理 `protagonist` 與 `major`，每位角色包含：
...
| 真人全身三視圖 | 16:10 | 核對身形比例與服裝連戲 |
| 表情九宮格 | 16:10 | 只改表情，不改身份 |
| 主要服裝與材質板 | 16:10 | 固定版型、色值、磨損與道具 |
| 電影感關鍵畫面 | 16:9 | 驗證人物進入故事場景後仍不換臉 |

完整規格見 `references/live-action-image-set.md` 與 `references/live-action-schema.md`。
...
| 卡通與真人影像提示詞不得出現角色名稱或別名 | 避免模型被名稱記憶與既有 IP 偏誤影響 |
| 分析欄位使用台灣繁體中文，模型提示詞依欄位使用英文 | 維持穩定、可重用的輸出格式 |
| `importance` 僅接受四種列舉值 | 保護資料結構相容性 |
| 真人版七張必要圖片、固定比例與輸出路徑必須完整 | 確保能直接交給製作流程 |
| 真人圖片 `PASS` 必須對應真實非空 PNG | 避免把送出生成誤報為完成 |
...
- 預設只為 `protagonist` 與 `major` 建立三視圖與真人圖片組。
- 角色分開產圖時仍可能發生身份或畫風漂移；真人模式必須先核准 `identity-board`，並作為其他圖片的主參考。
- 沒有圖像工具時不會產生 PNG，但 JSON、Markdown、HTML、三視圖提示詞與真人圖片組設定仍可正常交付。
...
自測涵蓋分塊、別名合併、資料驗證、台灣繁體中文檢查、Markdown／HTML 渲染、真人身份設定、必要圖片、人名洩漏與圖片檔案稽核；不呼叫模型，也不消耗模型額度。
